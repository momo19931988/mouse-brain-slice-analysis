        ...
        for file, img_stack in items:
            for ch_img in img_stack:
                ...
                records.append({
                    ...
                    "new_name": new_name,
                    "group_id": group_id,
                    "channel_index": ch_count,
                    "original_width": ch_img.shape[1],
                    "original_height": ch_img.shape[0],
                    "pad_left": pad_l,
                    "pad_top": pad_t,
                    "pad_right": pad_r,
                    "pad_bottom": pad_b,
                    "final_width": padded.shape[1],
                    "final_height": padded.shape[0]
                })
                ch_count += 1

    df = pd.DataFrame(records)
    df.to_csv(output_dir / "padding_info.csv", index=False)

    df['image_id'] = df['new_name'].apply(lambda x: x.split('_channel')[0])
    df[['image_id', 'group_id']].drop_duplicates().sort_values('image_id')\
      .to_csv(output_dir / "image_group_mapping.csv", index=False)
    print("✅ 映射表已保存")

    df.groupby('image_id')['channel_index'].count().reset_index(name='channel_count')\
      .to_csv(output_dir / "channel_count_summary.csv", index=False)
    print("✅ 通道数统计已保存")
...
def step4_main():
    ...
    for base in base_names:
        try:
            ...
            elastix.SetInitialTransform(sitk.CenteredTransformInitializer(
                template_sitk, ch_sitk[0], sitk.Euler2DTransform(), sitk.CenteredTransformInitializerFilter.GEOMETRY))
